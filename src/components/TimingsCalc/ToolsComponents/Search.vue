<template>
  <v-card>
    ...
    <v-virtual-scroll :items="results" item-height="48" max-height="240" min-height="0" bench="5">
      <template v-slot:default="{ item }">
        <v-list-item @click="search(item.id)">
          <v-list-item-avatar size="30">
            <v-avatar size="30" color="#808080">
              <img :src="require(`@/assets/img/maps/${id_to_name(item.m)}.png`)" />
            </v-avatar>
          </v-list-item-avatar>
          <v-list-item-content>
  ...
  computed: {
    results() {
      ...
      for (const [id, battlefield] of Object.entries(this.battlefields)) {
        if (battlefield.n.toLowerCase().startsWith(input)) results.push({ id: id, ...battlefield });
      }
